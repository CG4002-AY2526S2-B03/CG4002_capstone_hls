...
//   LAYER1 + heads (512->512, 512->256, etc.): inner loop pipelined II=4.
//     16 independent partial accumulators (s0..s15) break false resource conflicts.
//     float fadd has ~11-cycle latency; with 16 chains HLS achieves II=4, 2x fewer iters.
//
// Memory:
//   BRAM (rom_2p, cyclic=16): trunk_1, reg/cls_head_0 weights
//   LUTRAM: trunk_0, head_1 weights, biases, scalers
//   h_a/h_b/head_buf: partitioned cyclic=16, impl chosen by HLS (no BIND_STORAGE
//                     override -- after partition banks are small enough for FFs)
...
void pb_predict(
    ...
    // 16 independent partial accumulators, j+=16: 32 iterations.
    // II=4, 4x fewer iterations vs j+=4 -> ~4x speedup on this layer.
    // cyclic=16 BRAM provides 16 concurrent reads per cycle.
    LAYER1:
    for (int i = 0; i < HIDDEN; i++) {
    ...
    // STAGE 5: Regression Head Layer 0 (512 -> 256)
    REG_HEAD_L0:
    for (int i = 0; i < HEAD_HIDDEN; i++) {
        ...
        head_buf[i] = relu6((s0+s1+s2+s3+s4+s5+s6+s7+s8+s9+s10+s11+s12+s13+s14+s15) * reg_head_0_qscale + reg_head_0_bias[i]);
    }

    // STAGE 5b: Regression Head Layer 1 (256 -> 6) + inverse scaler
    REG_HEAD_L1:
    for (int i = 0; i < OUT_REG; i++) {
        ...
        for (int j = 0; j < HEAD_HIDDEN; j += 16) {
            #pragma HLS PIPELINE II=4
            s0  += (float)reg_head_1_weight_q[i*HEAD_HIDDEN+j+ 0] * head_buf[j+ 0];
            s1  += (float)reg_head_1_weight_q[i*HEAD_HIDDEN+j+ 1] * head_buf[j+ 1];
            s2  += (float)reg_head_1_weight_q[i*HEAD_HIDDEN+j+ 2] * head_buf[j+ 2];
            s3  += (float)reg_head_1_weight_q[i*HEAD_HIDDEN+j+ 3] * head_buf[j+ 3];
            s4  += (float)reg_head_1_weight_q[i*HEAD_HIDDEN+j+ 4] * head_buf[j+ 4];
            s5  += (float)reg_head_1_weight_q[i*HEAD_HIDDEN+j+ 5] * head_buf[j+ 5];
            s6  += (float)reg_head_1_weight_q[i*HEAD_HIDDEN+j+ 6] * head_buf[j+ 6];
            s7  += (float)reg_head_1_weight_q[i*HEAD_HIDDEN+j+ 7] * head_buf[j+ 7];
            s8  += (float)reg_head_1_weight_q[i*HEAD_HIDDEN+j+ 8] * head_buf[j+ 8];
            s9  += (float)reg_head_1_weight_q[i*HEAD_HIDDEN+j+ 9] * head_buf[j+ 9];
            s10 += (float)reg_head_1_weight_q[i*HEAD_HIDDEN+j+10] * head_buf[j+10];
            s11 += (float)reg_head_1_weight_q[i*HEAD_HIDDEN+j+11] * head_buf[j+11];
            s12 += (float)reg_head_1_weight_q[i*HEAD_HIDDEN+j+12] * head_buf[j+12];
            s13 += (float)reg_head_1_weight_q[i*HEAD_HIDDEN+j+13] * head_buf[j+13];
            s14 += (float)reg_head_1_weight_q[i*HEAD_HIDDEN+j+14] * head_buf[j+14];
            s15 += (float)reg_head_1_weight_q[i*HEAD_HIDDEN+j+15] * head_buf[j+15];
        }
    ...
    // STAGE 6: Classification Head Layer 0 (512 -> 256)
    CLS_HEAD_L0:
    for (int i = 0; i < HEAD_HIDDEN; i++) {
        #pragma HLS LOOP_FLATTEN off
        float s0=0.0f,s1=0.0f,s2=0.0f,s3=0.0f,s4=0.0f,s5=0.0f,s6=0.0f,s7=0.0f;
        ...
        head_buf[i] = relu6((s0+s1+s2+s3+s4+s5+s6+s7+s8+s9+s10+s11+s12+s13+s14+s15) * cls_head_0_qscale + cls_head_0_bias[i]);
    }

    // STAGE 6b: Classification Head Layer 1 (256 -> 6)
    CLS_HEAD_L1:
    for (int i = 0; i < OUT_CLS; i++) {
        ...
        for (int j = 0; j < HEAD_HIDDEN; j += 16) {
            #pragma HLS PIPELINE II=4
            s0  += (float)cls_head_1_weight_q[i*HEAD_HIDDEN+j+ 0] * head_buf[j+ 0];
            s1  += (float)cls_head_1_weight_q[i*HEAD_HIDDEN+j+ 1] * head_buf[j+ 1];
            s2  += (float)cls_head_1_weight_q[i*HEAD_HIDDEN+j+ 2] * head_buf[j+ 2];
            s3  += (float)cls_head_1_weight_q[i*HEAD_HIDDEN+j+ 3] * head_buf[j+ 3];
            s4  += (float)cls_head_1_weight_q[i*HEAD_HIDDEN+j+ 4] * head_buf[j+ 4];
            s5  += (float)cls_head_1_weight_q[i*HEAD_HIDDEN+j+ 5] * head_buf[j+ 5];
            s6  += (float)cls_head_1_weight_q[i*HEAD_HIDDEN+j+ 6] * head_buf[j+ 6];
            s7  += (float)cls_head_1_weight_q[i*HEAD_HIDDEN+j+ 7] * head_buf[j+ 7];
            s8  += (float)cls_head_1_weight_q[i*HEAD_HIDDEN+j+ 8] * head_buf[j+ 8];
            s9  += (float)cls_head_1_weight_q[i*HEAD_HIDDEN+j+ 9] * head_buf[j+ 9];
            s10 += (float)cls_head_1_weight_q[i*HEAD_HIDDEN+j+10] * head_buf[j+10];
            s11 += (float)cls_head_1_weight_q[i*HEAD_HIDDEN+j+11] * head_buf[j+11];
            s12 += (float)cls_head_1_weight_q[i*HEAD_HIDDEN+j+12] * head_buf[j+12];
            s13 += (float)cls_head_1_weight_q[i*HEAD_HIDDEN+j+13] * head_buf[j+13];
            s14 += (float)cls_head_1_weight_q[i*HEAD_HIDDEN+j+14] * head_buf[j+14];
            s15 += (float)cls_head_1_weight_q[i*HEAD_HIDDEN+j+15] * head_buf[j+15];
        }
